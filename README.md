...
DevBlog is a multi‑user publishing platform—comparable in scope to Medium or Dev.to—purpose‑built as a **distributed system, not a monolith**.  
It serves three roles (**Author**, **Reader**, **Admin**) and is designed for a target load of **100k+ monthly active users** and **10k–20k concurrent connections**.

### Why this exists

Most portfolio “blog” projects are a single Express/Next app with one Postgres database—enough to demonstrate CRUD, but not enough to demonstrate how real systems break and get designed around at scale. DevBlog is deliberately over‑engineered relative to what a blog needs, because the point isn’t the blog—it’s using the blog as a vehicle to make the same trade‑offs a backend/platform engineer makes on a real production system:
...
Every section below answers one of those questions concretely, with the actual trade‑off, not just the final choice.
...
- 💬 Threaded, nested comments modelled on a document store
- 🔎 Full‑text search with relevance ranking (English + Hindi)
- 📈 Trending & personalised feed generation with tiered cache TTLs
...
flowchart TB
    ...
    subgraph Infra_Helpers["Infrastructure"]
    ...
    style Infra_Helpers fill:#2e2e2e,color:#fff,stroke:#666
...
Request lifecycle — reading a personalised feed (cache‑aside)
...
Comment documents (MongoDB) — modelled as a tree rather than a flat table so an arbitrarily deep reply chain is one document read, not a recursive query:
...
Personalised feed	Redis	5 minutes	passive TTL expiry + post.published event (see sequence diagram above)
Trending list	Redis	1 hour	cron refresh (@nestjs/schedule) recomputes from a 7‑day rolling window
Post like count	Postgres (denormalised column)	—	kept in sync via a database trigger on the likes table, avoiding COUNT() on every read
Rate‑limit counters	Redis	sliding window	natural expiry
The general rule followed throughout: cache what’s expensive to compute and tolerant of being briefly stale (trending, personalised feeds), and never cache what must be correct on the next read (auth state, like counts a user just changed)—those are read from source or kept in sync structurally (the trigger), not cached at all.
...
Feed Service	Trending posts, personalised feed, cache orchestration	PostgreSQL + Redis
<details> <summary><strong>Expand for detailed per‑service design notes</strong></summary>
Auth Service
...
Post Service
Owns the canonical schema for users, posts, tags, and likes. Likes are denormalised onto the post row and kept in sync via a database trigger to avoid COUNT() on read. Row‑Level Security policies restrict draft visibility to the owning author; admins bypass via a dedicated role.

Comment Service
Comments are modelled as a document tree in MongoDB, which avoids recursive CTEs for arbitrarily deep reply chains. Validates the parent post’s existence via a gRPC call to the Post Service rather than a shared foreign key.
...
Feed Service
Generates the trending list from a 7‑day rolling window ranked by engagement; personalised feeds are computed from the follow graph. Cache TTLs are tiered—5 minutes for recent posts, 1 hour for trending—refreshed by a background cron job.
...
Authorization boundary: as covered in Design Decisions, the database (Row‑Level Security) is the actual enforcement point for “can this user see this draft,” not application code—application code adds UX‑friendly error messages on top of that, it isn’t the only gate.
...
Secrets: never committed—see Environment Variables and the .gitignore entries the setup script enforces (.env, .env.*, with .env.example explicitly allowed through).
...
Contract tests	gRPC calls between services are exercised against the compiled .proto types directly—a contract change that breaks a consumer fails at compile time (see API Contracts), which covers most of what a separate contract‑testing tool would otherwise catch
...
Prerequisites
Tool	Version	Notes
Node.js	20 LTS or newer	via nvm recommended
pnpm	9+	via corepack enable or npm i -g pnpm
Git	any recent	—
Docker + Docker Compose	any recent	for local Postgres / Mongo / Redis
...
OS	Required Terminal	Why
Linux (Ubuntu, Debian, Fedora, Arch, etc.)	Your default terminal (GNOME Terminal, Konsole, Alacritty, etc.) running bash or zsh.	No extra setup beyond Node.js + pnpm. This is the environment the script is written for.
macOS	Terminal.app or iTerm2, using the default zsh shell (bash also works).	Install Node via nvm or brew install node, then corepack enable for pnpm. macOS is Unix‑based, so the script runs natively—no additional layer needed.
Windows	WSL2 (Windows Subsystem for Linux) with an Ubuntu distro—not PowerShell, not CMD, not Git Bash.	The script relies on bash‑specific syntax (arrays, local, heredocs) and on native modules being compiled for a real Linux userland, which Git Bash cannot provide.
Windows — detailed WSL2 setup:
...
Launch the Ubuntu app from the Start menu—this is your terminal from here on.

Inside WSL2, install Node.js (using nvm) and pnpm:
...
nvm use 20
corepack enable
Place the setup-devblog.sh script in a WSL2 filesystem path (e.g. ~/Projects), not under /mnt/c/... (otherwise you’ll hit crippling I/O performance during npm install).
...
□ Complex query layer—trending, personalised feed, tag ranking
...
FAQ
Why not just use one Postgres database with well‑organised schemas instead of splitting into services at all?
...
Yes, and that’s addressed directly in Failure Modes: every event‑driven side effect has a non‑event fallback (TTL expiry or cron reconciliation), so losing an event degrades freshness, not correctness.

Why gRPC instead of just calling REST endpoints between services?
Covered in Design Decisions—typed, compiled contracts and HTTP/2 multiplexing matter more once internal call volume exceeds external request volume, which it does here (one client request fans out into several service calls).
...
Yes, deliberately—see Why this exists.
